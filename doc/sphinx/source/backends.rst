...
marshaling and calling the user kernel as an inline ``__device__`` function.
...
transfered from host to device prior to launching the kernel and which data
...
requires no CUDA-specific modifications and is automatically annonated with a
...
number of threads in parallel. Each thread is identified inside the kernel by its thread
...
the kernel signature are automatically annonated with OpenCL storage
